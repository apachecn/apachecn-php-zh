...
本附录显示了附录 B 和附录 C 中的代码版本，该代码将领域逻辑从页面脚本提取到*事务*类中。请注意，原始页面脚本现在如何简化为对象创建和注入机制，并将大部分逻辑交给*事务*类。还要注意`$failure`、`$credits`和`$article_types`变量现在是 Transactions 类上的属性，以及规范化/清理逻辑和信用计算逻辑是*事务*逻辑的一部分。
