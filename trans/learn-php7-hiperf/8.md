...
Composer 是一个 PHP 依赖项管理工具，它使我们能够为 PHP 应用定义依赖项，并由 Composer 安装/更新它们。Composer 完全是用 PHP编写的，是一个 PHP 归档（PHAR）格式的应用。
...
我们在[第 3 章](3.html "Chapter 3. Improving PHP 7 Application Performance")*中研究了 Grunt，以提高 PHP7 应用性能*。我们只使用它来合并 CSS 和JavaScript 文件并缩小它们。然而，Grunt 并非仅用于此目的。它是一个 JavaScript 任务运行程序，可以通过查看特定文件的更改或手动运行任务来运行任务。我们研究了如何手动运行任务，所以现在我们将研究在进行某些更改时如何使用 grunt watch 运行特定任务。
